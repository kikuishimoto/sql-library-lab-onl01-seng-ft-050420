...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM series JOIN books ON series.id = books.series_id JOIN character_books ON character_books.book_id = book.id JOIN characters ON character_books.character_id = character.id WHERE characters.species = 'human' GROUP BY series.title ORDER BY COUNT(*) DESC LIMIT 1"
end
